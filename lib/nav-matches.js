...
/**
 * Lien plat parmi des frères : la correspondance la plus longue gagne.
 */
export function isNavFlatItemActive(pathname, item, allItems) {
   if (item.items?.length) {
      return pathnameMatchesNavSection(pathname, item);
   }
   const siblings = allItems.filter((i) => !i.items?.length);
   return isNavSubItemActive(pathname, item, siblings);
}
