...
import { Loader2Icon, RefreshCwIcon, PackageIcon, ArrowUpCircleIcon, CheckCircle2Icon } from "lucide-react";
    ...
    <Card className="h-min">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          <span className="text-lg font-medium text-white">Extensions</span>
        </CardTitle>
        <CardAction>
          {loading ? (
        ...
        <p className="text-muted-foreground text-sm">
          ...
          {updatesCount > 0 ? (
            <span className="flex items-center gap-1.5 mt-2 text-amber-500 dark:text-amber-400">
              ...
              {updatesCount} mise{updatesCount > 1 ? "s" : ""} à jour à faire
            </span>
          ) : (
            total > 0 && (
